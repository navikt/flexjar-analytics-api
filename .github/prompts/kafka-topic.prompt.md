...
NOTE: `flexjar-analytics-api` does not currently use Kafka/Rapids & Rivers. Only apply this prompt if the repo is explicitly being extended with Kafka.

## Step 1: Add Kafka Configuration to Nais Manifest

Update `nais/app/dev.yaml` and `nais/app/prod.yaml` to include Kafka:
