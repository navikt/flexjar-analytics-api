...
- **Nais**: Examine manifests (in this repo: `nais/app/dev.yaml` and `nais/app/prod.yaml`)
...
The application uses Azure AD authentication via Nais with NAIS Texas sidecar introspection.

**Configuration**: `nais/app/dev.yaml` and `nais/app/prod.yaml`
- Azure AD application is enabled
- The app exposes operational endpoints under `/internal/*`

**Implementation**: `src/main/kotlin/no/nav/flexjar/config/Auth.kt`
- Bearer tokens are validated via Texas introspection
- A principal is constructed from introspected claims
