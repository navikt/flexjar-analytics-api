...
## Repo-specific Note (flexjar-analytics-api)

This repo validates tokens via **NAIS Texas sidecar introspection** and constructs a `BrukerPrincipal` from the returned claims.

✅ Prefer:
- `TexasClient.introspect(token)` and repo auth helpers
...
- Parsing JWTs directly for `azp_name` or group claims
...
kubectl exec -it <pod> -n <namespace> -- env | grep -E 'AZURE|NAIS_TOKEN_INTROSPECTION_ENDPOINT|FLEXJAR_ANALYTICS_CLIENT_ID'
