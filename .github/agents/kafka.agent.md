---
name: kafka-agent
description: Reference-only Kafka/Rapids & Rivers guidance (this repo currently does not use Kafka)
tools:
  - execute
  - read
  - edit
...
`flexjar-analytics-api` is an HTTP API backed by PostgreSQL and does **not** currently use Kafka or Rapids & Rivers.

Use this agent only as reference if the team explicitly decides to introduce Kafka/event-driven patterns.
...
- Treat this content as reference-only unless Kafka is explicitly adopted for this repo.
...
- Introducing Kafka/Rapids & Rivers in this repo (architecture change)
...
- Add Kafka/Rapids & Rivers dependencies to this repo without an explicit product/architecture decision.
