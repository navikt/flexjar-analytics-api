...
| Agent | Use For |
|-------|---------|
| `@auth-agent` | Auth in this repo (Texas introspection, principals, authorization plugins) |
| `@nais-agent` | accessPolicy, secrets, network policies |
| `@observability-agent` | Security alerts, anomaly detection |

## Repo-specific security notes (flexjar-analytics-api)

- Auth tokens are validated via NAIS Texas introspection; avoid adding in-app JWKS/JWT verification unless there is a clear migration plan.
- Operational endpoints are under `/internal/*` (`/internal/isAlive`, `/internal/isReady`, `/internal/prometheus`).
- Feedback content may contain PII; ensure log hygiene (never log raw feedback, tokens, or sensitive claims).
- Redaction happens server-side; preserve/extend the existing redaction patterns rather than adding ad-hoc filtering.
...
> **For detailed authentication implementation**, use `@auth-agent`.

### Authentication Strategy Overview

This repo primarily serves internal analytics endpoints and uses Azure AD access tokens validated via NAIS Texas introspection. TokenX is not part of the current implementation.
